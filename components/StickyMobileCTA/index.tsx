...
export default function StickyMobileCTA() {
  const handleClick = (e: React.MouseEvent, type: string) => {
    e.preventDefault();
    window.dispatchEvent(new CustomEvent('select-property-type', { detail: type }));
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-1000 bg-white shadow-[0_-4px_20px_rgba(0,0,0,0.15)] px-4 py-3 hidden max-md:block">
      ...
      <div className="flex gap-3">
        <CTAButton size="small" href="#header-form" className="flex-1 text-center py-3 px-2" onClick={(e) => handleClick(e, 'Une maison')}>Une maison</CTAButton>
        <CTAButton size="small" href="#header-form" className="flex-1 text-center py-3 px-2" onClick={(e) => handleClick(e, 'Un appartement')}>Un appartement</CTAButton>
      </div>
    </div>
  );
}
